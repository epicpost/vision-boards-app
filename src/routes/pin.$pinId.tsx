...
function PinDetail() {
  ...
  const sidePins = pins.filter((p) => p.id !== pin.id).slice(0, 10);
  const belowPins = pins.filter((p) => p.id !== pin.id).slice(10).concat(pins.filter((p) => p.id !== pin.id).slice(0, 10));
          ...
          <div className="flex gap-3 items-start">
            <div className="w-full xl:w-4/5 2xl:w-4/6">
              ...
              <div className="mt-3 columns-2 sm:columns-2 md:columns-3 lg:columns-4 gap-3 [column-fill:_balance]">
                {belowPins.map((p, i) => (
                  <PinCard key={`below-${i}-${p.id}`} pin={p} />
                ))}
              </div>
            </div>
